...
###
### get MEDIAN of population, derived from projected population density map, as integer and
### with a threshold of >1 to dismiss areas with very low population density.
pop_median <- median(as.integer(getValues(pop_proj))[as.integer(getValues(pop_proj))>1], na.rm = T)
###
pwe_pm10 <- (pm10*pop_proj)/pop_median
pwe_pm25 <- (pm25*pop_proj)/pop_median
###
